...
def play_audio(filename):
    """Play audio file using system-appropriate player."""
    try:
        if os.path.exists(filename):
            if sys.platform == "darwin":
                subprocess.run(["afplay", filename], check=True)
            elif sys.platform == "win32":
                # Using PowerShell as a quick cross-OS helper for Windows
                subprocess.run(["powershell", "-c", f"(New-Object Media.SoundPlayer '{os.path.abspath(filename)}').PlaySync()"], check=True)
            else:
                print(f"Play audio: {filename} (unsupported OS for auto-play)")
...
def select_language():
    ...
    while True:
        ...
        if choice in SUPPORTED_LANGUAGES:
            ...
            print(f"[OK] Selected: {lang_name} ({lang_code})")
            return lang_code, lang_name
        else:
            print("[FAIL] Invalid choice. Try again.")
    ...
    try:
        while True:
            ...
            if not transcript.strip() or transcript.startswith("[STT Error]"):
                print("[SKIP] No speech detected, skipping.")
